...
| R-014 | Auth 응답 필드명 정리 | `AuthResponse.name`에 실제로는 `nickname`이 들어가 있어 의미가 모호함 | low | todo | 인증 API 응답 스펙 정리 시 함께 반영 |
| R-015 | 인증 관련 상수 정리 | `Bearer` 같은 고정 문자열은 상수화하면 변경 포인트와 오타 위험을 줄일 수 있음 | low | todo | 보안/인증 코드 정리 단계에서 반영 |
| R-016 | 학습용 주석 밀도 정리 | `AuthService` 등 일부 클래스는 설명 주석이 많아 핵심 로직 가독성을 해칠 수 있음 | low | todo | MVP 완료 후 필요한 주석만 남기기 |
| R-017 | 인증 실패 메시지 운영용으로 통일 | 현재는 실행 확인을 위해 로그인 실패 메시지를 다르게 두고 있으므로, MVP 마무리 시 운영용 메시지 기준으로 정리할 필요가 있음 | medium | todo | 개발 단계에서는 현 상태 유지 |
| R-018 | 회원가입 후 응답 생성 흐름 정리 | 현재 `signup()`이 `login()`을 다시 호출해 응답을 생성하므로, 나중에 공통 응답 생성 메서드로 정리하면 불필요한 재조회와 검증을 줄일 수 있음 | low | todo | 인증 흐름 안정화 후 반영 |
| R-019 | 회원가입 중복 체크 방식 정리 | 현재는 사전 이메일 조회와 DB 유니크 예외 처리를 함께 사용 중이므로, MVP 이후 `existsByEmail` 사용 여부와 최종 중복 체크 기준을 정리할 필요가 있음 | low | todo | 동작 우선, 이후 단순화 검토 |
